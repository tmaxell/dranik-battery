...
dranik retrust             # снова доверять затвору после провала проверки
...
```bash
make install-app     # положить в /Applications и запускать при входе
make app-run         # просто запустить сборку из .build, ничего не устанавливая
make uninstall-app   # убрать приложение; демон при этом не трогается
```

Один значок и один попап: заряд с отметкой лимита, одна строка о том, почему
машина сейчас заряжается или нет, слайдер лимита и выключатель. Всё остальное
осталось в CLI намеренно.

Приложению не нужны ни root, ни Developer ID, ни Xcode: демон уже открыл сокет
группе `admin`, а бандл собирается из Command Line Tools. Логика целиком в демоне —
приложение только показывает и просит.

```bash
DranikApp --check    # что приложение видит прямо сейчас, текстом
```
...
- Если проверка применения дважды подряд и близко по времени противоречит записи — ограничение выключается и об этом говорят громко: в логе `fault`, в `dranik daemon`, в попапе. Возврат — `dranik retrust`, осознанным действием, а не по таймеру.
...
Sources/
  ...
  DranikApp/        Приложение в меню-баре. Ни одного решения — только показ
  dranik/           CLI
...
```bash
make build
make test              # 224 теста, 703 утверждения
make ui-drill          # приложение против демона, который врёт нарочно
make ui-snapshots      # отрисовать все состояния попапа в PNG, светлую и тёмную
make daemon-dry-run    # весь цикл решений, ничего не записывая
```

`make ui-snapshots` рисует попап в невидимое окно и сохраняет каждое состояние
картинкой — включая переключатель и слайдер, которые `ImageRenderer` подменяет
заглушками. Единственный способ посмотреть на «затвор перестал отвечать», не дожидаясь,
пока он и правда перестанет.

`make ui-drill` поднимает настоящий `ControlServer` на временном сокете, скармливает
приложению отчёты, которых здоровый демон не выдаст — потерянное доверие к затвору,
железо без затвора, устаревшее показание, сокет без прав, — и читает обратно,
что приложение из этого сделало. Тест не может посмотреть на экран; на текст может.
